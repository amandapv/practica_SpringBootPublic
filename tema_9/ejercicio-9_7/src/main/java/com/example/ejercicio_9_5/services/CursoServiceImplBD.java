package com.example.ejercicio_9_5.services;
...
import com.example.ejercicio_9_5.repositories.CursoReporitory;
...
public class CursoServiceImplBD implements CursoService{
    ...
    public List<Curso> obtenerTodos() {
        ...
        List<Curso> listaCursos =  repositorio.findAll(Sort.by(Sort.Direction.ASC, "nombre"));
        if (listaCursos.isEmpty()) {
            throw new EmptyCursosException();
        }
        return listaCursos;
    }
    ...
    public Curso obtenerPorId(long id) {
        Curso curso = repositorio.findById(id).orElseThrow( ()-> new CursoNotFoundException(id) ); //findById devuelve el curso con ese ID en concreto (si lo ha encontrado; en este caso si no lo encuentra llamo a mi excepción personalizada)
    ...
    public List<Curso> buscarPorNombre(String textoNombre) {
        List<Curso> listaCursos = repositorio.findByNombreContainingIgnoreCase(textoNombre);
        if (listaCursos.isEmpty()) {
            throw new EmptyCursosException();
        }
        return listaCursos;
    ...
    public List<Curso> buscarPorTematica(Tematica tematica) {
        List<Curso> listaCursos = repositorio.findByTematica(tematica);
        if (listaCursos.isEmpty()) {
            throw new EmptyCursosException();
        }
        return listaCursos;
    ...
    public List<Curso> filtrarImporteMenorIgualPrecio(Double precio) {
        List<Curso> listaCursos = repositorio.findByPrecioLessThanEqual(precio);
        if (listaCursos.isEmpty()) {
            throw new EmptyCursosException();
        }
        return listaCursos;
